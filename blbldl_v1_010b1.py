...
def get_file_from_cmd(link):
    ###把路径cd到./result文件夹
    exe_path = r'aria2c'
    order = exe_path + ' -s18 -x10 ' + link + ' --referer "https://www.bilibili.com" --file-allocation none'
    os.system(order)
...
def blbldl(durl,filename=""):
    ...
    if info_p>=1:
        ...
        ###整合 使用ffmpeg
        os.system('ffmpeg -i ' + videoname + ' -i ' + audioname + ' -c:v copy -c:a copy ' + filename)  # combine video and audio
        ###删除原分段下载文件
        del_func(videoname)
        del_func(audioname)
    else:
        ###整合 使用ffmpeg
        os.system('ffmpeg -i ' + videoname  + ' -c:v copy ' + filename)  # combine video and audio
        ###删除原分段下载文件
        del_func(videoname)
    ###结束
...
if __name__ == '__main__':
    try:
        ...
        if category_bv:
            ...
            for i in eplist:
                vUrl = 'https://api.bilibili.com/x/player/playurl?cid=' + str(i.get('cid', '')) + '&bvid=' +category_bv.group(0) + '&qn=120&fnval=80'
                blbldl(vUrl ,category_bv.group(0)+"_"+str(i.get('ep', 0)))
            ending()
        del category_bv
        ...
        if category_av:
            ...
            for i in eplist:
                vUrl = 'https://api.bilibili.com/x/player/playurl?cid=' + str(i.get('cid', '')) + '&avid=' + re.search("[^a|v][0-9]*" ,category_av.group(0) , re.I).group(0) + '&qn=120&fnval=80'
                print(vUrl)
                blbldl(vUrl, category_av.group(0) + "_" + str(i.get('ep', 0)))
            ending()
        del category_av
        ...
        if category_ep:
            if not re.search(r"bilibili.com", url, re.I):
                ...
                blbldl(url ,category_ep.group(0))
            ending()
    except Exception as e:
